...
check_2_1() {
  local id="2.1"
  local desc="以非root用户运行docker进程 (Manual)"
...
check_2_2() {
  local id="2.2"
  local desc="确保限制默认网桥上容器之间的网络流量 (Scored)"
...
check_2_3() {
  local id="2.3"
  local desc="确保日志级别设置为INFO (Scored)"
...
check_2_4() {
  local id="2.4"
  local desc="确保允许docker更改iptables (Scored)"
...
check_2_5() {
  local id="2.5"
  local desc="确保不使用不安全的镜像仓库 (Scored)"
...
check_2_6() {
  local id="2.6"
  local desc="确保不使用aufs存储驱动程序 (Scored)"
...
check_2_7() {
  local id="2.7"
  local desc="确保Docker守护进程配置了TLS身份认证 (Scored)"
...
check_2_8() {
  local id="2.8"
  local desc="确保配置了合适的ulimit (Manual)"
...
check_2_10() {
  local id="2.10"
  local desc="确保使用默认的cgroup (Scored)"
...
check_2_11() {
  local id="2.11"
  local desc="确保设置容器的默认空间大小 (Scored)"
...
check_2_12() {
  local id="2.12"
  local desc="确保启用Docker客户端命令的授权 (Scored)"
...
check_2_13() {
  local id="2.13"
  local desc="确保配置集中式和远程日志记录 (Scored)"
...
check_2_16() {
  local id="2.16"
  local desc="确保禁用userland代理 (Scored)"
...
check_2_17() {
  local id="2.17"
  local desc="确保应用守护进程范围的自定义seccomp配置文件 (Manual)"
...
check_2_18() {
  ...
  local id="2.18"
  local desc="确保生产环境中避免实验性功能 (Scored)"
