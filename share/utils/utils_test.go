...
func TestIPEnclosure(t *testing.T) {
	cases := [][]string{
		[]string{"192.168.1.0/24", "192.168.1.1", "192.168.1.20", "192.168.1.255"},
		[]string{"192.168.0.0/22", "192.168.1.1", "192.168.2.1"},
		[]string{"192.168.1.0/31", "192.168.1.0", "192.168.1.1"},
		[]string{"192.168.1.0/30", "192.168.1.1", "192.168.1.2"},
		[]string{"192.168.1.2/31", "192.168.1.2", "192.168.1.3"},
		[]string{"192.0.0.0/2", "192.168.1.1", "255.255.255.255"},
		[]string{"192.168.1.1/32", "192.168.1.1", "192.168.1.1"},
		[]string{"10.1.1.2/31", "10.1.1.2", "10.1.1.3"},
		[]string{"0.0.0.0/4", "10.1.1.2", "0.0.0.0"},
		[]string{"0.0.0.0/0", "192.1.1.2", "0.0.0.0"},
	}
	for _, v := range cases {
		ips := make([]net.IP, 0)
		for _, ip := range v[1:] {
...
func TestSubnetContains(t *testing.T) {
	...
	cases := []testCase{
		testCase{"10.1.0.0/16", "10.1.0.0/24", true, 1},
		testCase{"10.1.1.0/24", "10.1.0.0/24", false, 0},
		testCase{"10.1.1.192/28", "10.1.1.0/24", true, -1},
		testCase{"10.1.1.0/24", "10.1.1.0/24", true, 0},
		testCase{"10.1.2.3/32", "10.1.2.0/25", true, -1},
		testCase{"0.0.0.0/0", "10.1.1.0/24", true, 1},
	}
...
func TestSubnetLoose(t *testing.T) {
	...
	cases := []testCase{
		testCase{"1.2.3.4/16", share.CLUSIPAddrScopeNAT, "1.2.0.0/16"},
		testCase{"1.2.3.4/24", share.CLUSIPAddrScopeNAT, "1.2.3.0/24"},
		testCase{"1.2.3.4/16", share.CLUSIPAddrScopeGlobal, "1.2.0.0/16"},
		testCase{"1.2.3.4/24", share.CLUSIPAddrScopeGlobal, "1.2.3.0/24"},
		testCase{"1.2.3.4/32", share.CLUSIPAddrScopeGlobal, "1.2.3.0/24"},
	}
	for _, c := range cases {
