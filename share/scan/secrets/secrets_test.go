...
func TestSampleFiles(t *testing.T) {
	...
	var myFileSpec []FileType = []FileType{
		FileType{Description: "test files", Expression: `(.*?)(yaml|key|txt)`},
	}

	////
	var myRules []Rule = []Rule{
		Rule{Description: "AWS.Manager.ID", Expression: `(A3T[A-Z0-9]|AKIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}`, Tags: []string{"key", "AWs"}},
	}
