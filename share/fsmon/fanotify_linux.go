...
func (fn *FaNotify) isFileException(bNeuvectorSvc bool, path string, pInfo *ProcInfo, mask uint32) bool {
	if bNeuvectorSvc {
		...
		if mask == 0 || (mask&syscall.IN_CREATE) != 0 {
			if (path == "/etc/secvector/certs/internal/ca.cert") ||
				(path == "/etc/secvector/certs/internal/cert.key") ||
				(path == "/etc/secvector/certs/internal/cert.pem") {
				log.WithFields(log.Fields{"path": path}).Info("FMON: important files")
				return false // warnings
			}

			if dir == "/etc/secvector/certs" || dir == "/etc/secvector/certs/internal" {
